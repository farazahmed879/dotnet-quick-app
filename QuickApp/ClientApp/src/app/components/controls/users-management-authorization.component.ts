...
@Component({
  selector: 'app-users-management-authorization',
  templateUrl: './users-management-authorization.component.html',
  styleUrls: ['./users-management-authorization.component.scss']
})
export class UsersManagementAuthorizationComponent implements OnInit, AfterViewInit {
  columns: any[] = [];
  rows: User[] = [];
  rowsCache: User[] = [];
  editedUser: UserEdit;
  ...
  ngOnInit() {
    ...
    this.columns = [
      ...
      { prop: 'Account Name/Login', name: gT('users.management.Title'), width: 50 },
      { prop: 'Account Owner', name: gT('users.management.UserName'), width: 90, cellTemplate: this.userNameTemplate },
  ...
  newGroupInfo() {
   ...
   // this.editedGroupInfo = this.createOrEditGroupModal.newGroup();
