...
export class CreateOrEditUserComponent implements OnInit {
  ...
  save(form: NgForm) {
    this.isSaving = true;
    this.user["createdBy"] = this.currentUserId();
    this.user["userID"] = this.currentUserId();
    this.alertService.startLoadingMessage('Saving changes...');
    console.log(this.user);
    if (this.isNewUser) {
      this.accountService.newUser(this.user).subscribe(user => this.saveSuccessHelper(user), error => this.saveFailedHelper(error));
    } 
  }
  ...
  handleSignatureUpload(files: FileList) {
    this.getBase64(files.item(0)).then((res) => {
